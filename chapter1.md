...
{%madoko%}
Package         : [heading-base={&heading-base;},section-num={&tex-section-num;},bib-label={&bib-label;}]madoko2
Logo            : False
Tex2            : True
Maketitle       : False
Heading Depth   : 3 
...
Snippet     { input: math; display:block; snippet-needpdf: true }
...
@if not(cite-style~=/[nN]umeric/) {
  Bib Label: False
}

@if not( tex ) {
  Bibliography { before: "# &name-references; { -; .heading-references }&nl;" }
}
@if not( tex ) and bib-label {
  Bibitem      { before: '[\[&cite-label;\]\ \ ]{.bibitem-before}' }
}
...
Bibliography { tex-env: "thebibliography{&caption;}"; tex-cmd-after: "\par"; tex-cmd-outer-before: "\def\refname{&name-references;}%mdk&nl;"  }
